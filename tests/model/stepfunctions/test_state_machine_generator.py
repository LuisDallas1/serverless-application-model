...
class StepFunctionsStateMachine(TestCase):
    ...
    def test_state_machine_multiple_definition_sources(self):
        self.kwargs["definition"] = DefinitionConfig(
            definition={"StartAt": "StateOne", "States": {"StateOne": {"Type": "Pass", "End": True}}},
            definition_uri="s3://my-sam-bucket/definition.asl.json",
        )
        with self.assertRaises(InvalidResourceException) as error:
            StateMachineGenerator(StateMachineConfig(**self.kwargs)).to_cloudformation()
        self.assertEqual(
    ...
    def test_state_machine_no_role_or_policies(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="s3://my-demo-bucket/my_asl_file.asl.json")
    ...
    def test_state_machine_both_role_and_policies(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="s3://my-demo-bucket/my_asl_file.asl.json")
        self.kwargs["role"] = RoleConfig(
            role="arn:my-sample-role",
            policies={
    ...
    def test_state_machine_invalid_definition_uri_string(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="invalid_uri")
        with self.assertRaises(InvalidResourceException) as error:
            StateMachineGenerator(StateMachineConfig(**self.kwargs)).to_cloudformation()
        self.assertEqual(
            error.exception.message,
    ...
    def test_state_machine_invalid_definition_uri_object(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri={"Bucket": "only-bucket-name"})
        with self.assertRaises(InvalidResourceException) as error:
            StateMachineGenerator(StateMachineConfig(**self.kwargs)).to_cloudformation()
        self.assertEqual(
            error.exception.message,
    ...
    def test_state_machine_no_tags_provided(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="s3://mybucket/myASLfile")
        self.kwargs["role"] = RoleConfig(role="my-test-role-arn")
    ...
    def test_state_machine_with_alias_as_event_source_target(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="s3://mybucket/myASLfile")
        self.kwargs["role"] = RoleConfig(role="my-test-role-arn")
        self.kwargs["deployment"] = DeploymentConfig(
            use_alias_as_event_target=True,
    ...
    def test_state_machine_with_alias_as_event_source_target_requires_alias(self):
        ...
        # Missing property: auto_publish_alias not set
        with self.assertRaises(InvalidResourceException) as error:
            StateMachineGenerator(StateMachineConfig(**self.kwargs)).to_cloudformation()
        self.assertEqual(
            error.exception.message,
    ...
    def test_state_machine_with_auto_publish_alias_and_no_deployment_preference(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="s3://mybucket/myASLfile")
        self.kwargs["role"] = RoleConfig(role="my-test-role-arn")
        self.kwargs["deployment"] = DeploymentConfig(auto_publish_alias="live")
        generated_managed_traffic_shifting_resources = StateMachineGenerator(
            StateMachineConfig(**self.kwargs)
        )._generate_managed_traffic_shifting_resources()
        self.assertEqual(
    ...
    def test_state_machine_with_deployment_preference_and_no_auto_publish_alias(self):
        self.kwargs["definition"] = DefinitionConfig(definition_uri="s3://mybucket/myASLfile")
        self.kwargs["role"] = RoleConfig(role="my-test-role-arn")
        self.kwargs["deployment"] = DeploymentConfig(deployment_preference={"Type": "ALL_AT_ONCE"})
        with self.assertRaises(InvalidResourceException) as error:
            StateMachineGenerator(StateMachineConfig(**self.kwargs)).to_cloudformation()
        self.assertEqual(
            error.exception.message,
