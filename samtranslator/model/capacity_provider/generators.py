...
class CapacityProviderGenerator:
    ...
    def to_cloudformation(self) -> list[Resource]:
        ...
        if not self.config.operator_role:
            ...
            # 2. Pass ARN to capacity provider resource via self.config.operator_role
            self.config.operator_role = fnGetAtt(operator_iam_role.logical_id, "Arn")
    ...
    def _create_capacity_provider(self) -> LambdaCapacityProvider:
        ...
        capacity_provider = LambdaCapacityProvider(
            self.logical_id, depends_on=self.config.depends_on, attributes=self.config.resource_attributes
        )
        ...
        if self.config.operator_role:
            capacity_provider.PermissionsConfig = {"CapacityProviderOperatorRoleArn": self.config.operator_role}

        # Set the Tags - always add SAM tag, plus any user-provided tags
        capacity_provider.Tags = self._transform_tags(self.config.tags)

        # Set the InstanceRequirements if provided
        if self.config.instance_requirements:
            capacity_provider.InstanceRequirements = self._transform_instance_requirements()

        # Set the ScalingConfig if provided
        if self.config.scaling_config:
        ...
        if self.config.kms_key_arn:
            capacity_provider.KmsKeyArn = self.config.kms_key_arn

        # Set PropagateTags from ManagedResourceTags if provided
        if self.config.managed_resource_tags:
            capacity_provider.PropagateTags = self._transform_managed_resource_tags()

        # Pass through resource attributes
        if self.config.passthrough_resource_attributes:
            for attr_name, attr_value in self.config.passthrough_resource_attributes.items():
    ...
    def _transform_instance_requirements(self) -> dict[str, Any]:
        ...
        if self.config.instance_requirements.get("Architectures") is not None:
            instance_requirements["Architectures"] = self.config.instance_requirements["Architectures"]

        if self.config.instance_requirements.get("AllowedTypes") is not None:
            instance_requirements["AllowedInstanceTypes"] = self.config.instance_requirements["AllowedTypes"]

        if self.config.instance_requirements.get("ExcludedTypes") is not None:
            instance_requirements["ExcludedInstanceTypes"] = self.config.instance_requirements["ExcludedTypes"]
    ...
    def _transform_scaling_config(self) -> dict[str, Any]:
        ...
        if self.config.scaling_config.get("MaxVCpuCount") is not None:
            scaling_config["MaxVCpuCount"] = self.config.scaling_config["MaxVCpuCount"]

        # Handle AverageCPUUtilization structure
        if self.config.scaling_config.get("AverageCPUUtilization") is not None:
            ...
            scaling_policies.append(
                {
                    ...
                    "TargetValue": self.config.scaling_config["AverageCPUUtilization"],
                }
            )
    ...
    def _transform_managed_resource_tags(self) -> dict[str, Any]:
        ...
        """
        tags: dict[str, Any] = self.config.managed_resource_tags or {}
